  ...
  for (let i = 0; i < completedCount; i++) {
    ...
    const tournament = await prisma.tournament.create({
      data: {
        ...
        bracket: generateBracketStructure(randomChoice([16, 32, 64])),
      },
    });
...
function generateBracketStructure(playerCount: number) {
  ...
  for (let round = 1; round <= rounds; round++) {
    const matchesInRound = Math.ceil(playerCount / Math.pow(2, round));
    const roundMatches: any[] = [];

    for (let match = 1; match <= matchesInRound; match++) {
      roundMatches.push({
        id: `round${round}_match${match}`,
        player1: `player${match * 2 - 1}`,
        player2: `player${match * 2}`,
